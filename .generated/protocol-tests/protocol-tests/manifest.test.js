...
import { canonicalizeManifest, signManifest, validateManifestFreshness, validateStoredSignature, } from "../src/protocol/manifest.js";
const BASE_MANIFEST = {
...
test("signManifest matches the backend deterministic sha256 signature", async () => {
    ...
    assert.equal(canonicalizeManifest(signed), '{"server_id":"signed-server","identity":"remote-signer","version":1,"description":"signed","updated_at":"2026-01-01T00:00:00Z","capabilities":{"chat":true,"voice":false}}');
...
test("validateManifestFreshness rejects stale manifests when a max age is enforced", () => {
    validateManifestFreshness({ ...BASE_MANIFEST, identity: "remote-signer", signature: EXPECTED_SIGNATURE }, new Date("2026-01-01T00:04:59Z"), 5 * 60_000);
    assert.throws(() => validateManifestFreshness({ ...BASE_MANIFEST, identity: "remote-signer", signature: EXPECTED_SIGNATURE }, new Date("2026-01-01T00:06:00Z"), 5 * 60_000), /manifest is stale/);
});
