...
export const ChatArea: React.FC<ChatAreaProps> = ({ 
  ...
  // Local state for messages to handle immediate UI updates for reactions
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
        ...
        const newMessage: Message = {
            ...
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
            attachments: ['file'] // Simplified for mock
        };
        setMessagesState(prev => [...prev, newMessage]);
    }
  };
  ...
  const handleReactionToggle = (msgId: string, emoji: string) => {
    setMessagesState(prev => prev.map(msg => {
        ...
        if (existing) {
            if (existing.reacted) {
                // Remove reaction logic (decrement or remove)
                // For demo, if count becomes 0, we remove it
  ...
  const highlightText = (text: string, query: string) => {
    ...
    // Escape regex special characters to prevent runtime errors with special inputs
      ...
      <div className={`absolute inset-0 overflow-y-auto px-4 md:px-12 pt-24 pb-32 ${
        ...
        {filteredMessages.map((msg) => {
          ...
          return (
            ...
            >
              {/* Accent strip variant (canonical spec 4.8) */}
