...
import { negotiateCapabilities, negotiateConversationSecurityMode, validFeatureFlagName, } from "../src/protocol/capabilities.js";
...
test("negotiateCapabilities accepts supported flags and reports missing required flags", () => {
    const result = negotiateCapabilities(["cap.chat", "cap.voice", "cap.identity"], ["cap.chat", "cap.future", "bad"], ["cap.identity", "cap.sync", "cap.voice..opus"]);
...
test("negotiateConversationSecurityMode chooses the first shared preferred mode", () => {
    assert.deepEqual(negotiateConversationSecurityMode(["seal", "tree"], ["tree", "clear"]), { mode: "tree", reason: "matched" });
    assert.deepEqual(negotiateConversationSecurityMode(["seal"], ["clear"]), { mode: "unspecified", reason: "unsupported-mode" });
    assert.deepEqual(negotiateConversationSecurityMode(["seal"], []), { mode: "unspecified", reason: "no-offer" });
});
