...
export async function signManifest(manifest, identity, digest) {
...
export async function validateManifestSignature(manifest, identity, digest) {
    if (isXoreinManifest(manifest)) {
...
export async function validateStoredSignature(manifest, digest) {
    validateManifestFields(manifest);
    if (isXoreinManifest(manifest)) {
...
export function validateManifestFreshness(manifest, reference = new Date(), maxAgeMs = 0) {
    validateManifestFields(manifest);
...
function isXoreinManifest(manifest) {
    return Array.isArray(manifest.capabilities);
}
function canonicalizeXoreinManifest(manifest) {
    validateManifestFields(manifest);
...
async function validateXoreinManifestSignature(manifest) {
...
export async function sha256Hex(payload, digest) {
    if (digest) {
